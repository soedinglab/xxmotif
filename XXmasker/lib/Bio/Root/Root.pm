...
#   return $VERBOSITY unless ref $self;
   # for below, we have
   # Bio::Root::Root::verbose()  # returns $VERBOSITY
   # Bio::Root::Root::verbose(1) # sets global $VERBOSITY to 1
   # $obj->verbose(1)            # sets $obj->{'_root_verbose'} as before
   # $obj->verbose()             # now returns $VERBOSITY unless 
   #                             # $obj->verbose($value) called previously
   #                             
   for ($self) {
       !defined && do {
	   return $VERBOSITY;
       ...
       ref && do {
	   return $self->{'_root_verbose'} = $value if (defined $value);
	   return $self->{'_root_verbose'};
       };
       do { # $self is a defined scalar, set global
	   return $VERBOSITY = $self;
       };
   } # for $self
   throw("Whoa...logic warp..."); 
}
